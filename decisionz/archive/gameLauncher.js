...
function updateTimeDiv(){
	...
	$("#currentTime").html("Day " + parseInt(timeParts[3]) + " &nbsp; " +
						parseInt(timeParts[4]) + ":" + timeParts[5] +
						":" + timeParts[6]);
...
function constructPage(){
	...
	if(checkDecisionVarI("narration", "true")){
		$("#narrationPlayerDiv").empty().append($('<audio id="narrationAudioPlayer" width="0" autoplay height="0"' +
   	...
   	/*audio_play_file(jCurrentScene.attr("name") + '-' + jCurrentPage.attr("id") , 
	            					"assets/narration/" );
	
	document.getElementById('audioPlayer').play();*/

	if(checkDecisionVarI("narration", "true") && false){
   		
		var htmlAudioPlayer = document.getElementById('narrationAudioPlayer');
	
		//htmlAudioPlayer.innerHTML = ""; 
   		
   		var sourceList = "";
		if(htmlAudioPlayer.canPlayType('audio/wav') == "true" || 
			htmlAudioPlayer.canPlayType('audio/wav') == "maybe" ){
			sourceList = sourceList + '<source src="' + 'assets/narration/wav/' +
								jCurrentScene.attr("name") + '-' + 
	            				jCurrentPage.attr("id") + '.wav' + '" type="audio/wav">';
		}
		
		/*if(htmlAudioPlayer.canPlayType('audio/ogg') == "true" || 
			htmlAudioPlayer.canPlayType('audio/ogg') == "maybe" ){
			sourceList = sourceList + '<source src="assets/music/1.ogg" type="audio/ogg">';
		}*/
		
		htmlAudioPlayer.innerHTML = sourceList; 
		//setTimeout(function(){htmlAudioPlayer.play()},3000);

   		
   		/*$("#narrationAudioPlayer").attr("src", 'assets/narration/wav/' +
	            											jCurrentScene.attr("name") + '-' + 
	            											jCurrentPage.attr("id") + '.wav')*/
	            											
   		//document.getElementById("narrationAudioPlayer").load();
...
function playPauseBtnClicked(){
  if(document.getElementById('musicAudioPlayer').paused){
      $("#playPauseBtn").text("\"")
      unpauseAudio()
  }else{
      $("#playPauseBtn").text(">")
